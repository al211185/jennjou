...
export default function CurriculumSection() {
  return (
    <section
      ...
      data-tone="curriculum"
      className="relative flex min-h-screen w-full items-start justify-center border-y border-black px-4 py-12 text-black sm:px-6"
    >
      <div className="section-shell reveal-up mx-auto flex h-[82vh] w-full max-w-6xl flex-col space-y-8 overflow-y-auto px-5 py-6 sm:px-8 sm:py-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-lg text-gray-700">
                AKA. jennjou · Diseñador digital de medios interactivos
              </p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs font-semibold">
              {[
                ...
                <span key={tag} className="chip px-3 py-1">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-xl border border-black/20 bg-white/75 p-4 text-sm sm:gap-4 sm:p-5 md:max-w-xl">
            <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.6fr_1fr]">
          <div className="space-y-6 rounded-3xl border border-black/20 bg-white/72 p-6">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-xl font-semibold">Experiencia profesional</h3>
              <span className="chip px-3 py-1 text-xs uppercase tracking-wide">
                2025
              </span>
            </div>

            <div className="relative space-y-4 border-l border-dashed border-gray-400 pl-6">
              {experiences.map((experience) => (
                <div
                  key={experience.title + experience.period}
                  className="relative rounded-2xl border border-black/15 bg-gray-50/90 p-4"
                >
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-black/15 bg-gray-100/90 p-4">
              ...
              <div className="rounded-2xl border border-black/15 bg-gray-100/90 p-4">
                ...
                <p className="mt-2 text-sm text-gray-700">
                  ...
                  inmersivas y material educativo interactivo.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 lg:space-y-6">
            <div className="rounded-2xl border border-black/20 bg-white/85 p-5">
              <h3 className="text-lg font-semibold">Habilidades</h3>
              <div className="mt-3 flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill} className="chip px-3 py-1 text-xs font-semibold">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-black/20 bg-white/85 p-5">
              ...
              <ul className="mt-3 space-y-3 text-sm text-gray-700">
                {languages.map((language) => (
                  <li
                    key={language.name}
                    className="flex items-center justify-between rounded-xl border border-black/10 bg-gray-50 px-3 py-2"
                  >
                    <span className="font-medium">{language.name}</span>
                    <span className="chip px-2 py-0.5 text-xs font-semibold">
                      {language.level}
                    </span>
                  </li>
                ))}
            ...
            <div className="rounded-2xl border border-black/20 bg-white/85 p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {tools.map((tool) => (
                  <span key={tool} className="chip px-3 py-1 text-xs font-semibold">
                    {tool}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-black/20 bg-white/85 p-5">
              ...
              <p className="mt-2 text-sm text-gray-700">
                ...
                Siempre buscando mezclar creatividad con experiencias digitales
                claras y útiles.
              </p>
            </div>
          </div>
